...
| | 路徑 | |
|---|---|---|
| **Playground** | `/playground` | 網頁聊天（同一批渠道）；對話存 D1；SSE 串流、上游身分淨化。匿名**體驗模式**是 fail-closed —— 與會員路徑 fail-open 的配額刻意相反（[ADR-0009](./docs/adr/0009-demo-mode-fail-closed.md)）。 |
| **內容門戶** | `/news` `/articles` `/p/{slug}` | SSR 新聞／文章系統：圖片存 D1、RSS、sitemap、OG/JSON-LD、自訂頁面。 |
| **VPN 訂閱** | `/vpn` | 多上游合併成一條會員網址；沒有這項權限的人連它存在都看不到。 |
| **工具** | `/` `/ip` `/ua` | 最早的 IP／UA 查詢 SPA。 |
| **管理** | `/settings` `/members` `/admin` `/logs` `/api-docs` | API 能設的，網頁全能設：配額、體驗模式、定價、Telegram 告警、自訂頁面；會員／服務管理；訪客＋錯誤＋用量（含成本）儀表板。 |
