...
(function(){
  ...
  // 站長的聯絡方式（閘門畫面的「聯絡我」小鈕；2026-07-14 起取代原本的大 emoji 圖示）
  var CONTACT_URL="https://www.facebook.com/share/1CUYCqQAmo/?mibextid=wwXIfr";
  function contactBtn(){
    var a=el("a","gcontact",tx("聯絡我","Contact me"));
    a.href=CONTACT_URL;a.target="_blank";a.rel="noopener noreferrer";
    return a;
  }
  // 登入閘門畫面（未登入時）
  ...
  function gatePending(host,user){
    ...
    var g=el("div","gate");
    g.appendChild(el("h2",null,tx("等待站長批准","Waiting for approval")));
    g.appendChild(el("p",null,tx("你已經用 "+user.email+" 登入了，但這個服務需要站長批准後才能使用。批准後回來重新整理即可。","You're signed in as "+user.email+", but this service needs the site owner's approval first.")));
    g.appendChild(contactBtn());
    host.appendChild(g);
  }
  function flash(msg){
