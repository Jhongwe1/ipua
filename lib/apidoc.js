...
// ⚠ 新增或修改任何 API 時，記得同步更新：這份文件＋.claude/skills/uaip-api/SKILL.md（給 agent 的版本）。
// 注意：這個字串是 template literal — 行內程式碼的反引號要寫成 \`（跳脫），程式碼區塊用 ~~~ 圍欄。
// Markdown 語法範例（![說明](…)、[文字](…)、**粗體**）一定要包在行內程式碼裡，
// 不然 /api-docs 渲染時會變成真的圖片／連結（2026-07-09 踩過：/img/9 整張圖跑進文件頁）。
...
- 管理金鑰＝Cloudflare Pages 環境變數 **LOGS_TOKEN**，跟 /logs、/admin 網頁登入同一把；目前的值記在專案根目錄 ADMIN.md（不會上網）。
