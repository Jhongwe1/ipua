...
export function siteBrand(env, request) {
  try { return new URL(siteOrigin(env, request)).hostname; } catch (e) { return ""; }
}
...
// ===== CSP（2026-07-14 v1.0.0）=====
// SSR 頁面全部經過 html() 這個單一入口 → 每個回應產一顆 nonce、
// 用 regex 蓋章到 body 裡所有 <script 標籤，並送出對應的 CSP 標頭。
...
export function html(body, status) {
  ...
  const stamped = String(body).replace(/<script(?=[\s>])/g, '<script nonce="' + nonce + '"');
...
export function pageShell(o) {
  ...
  return '<!DOCTYPE html>\n<html lang="zh-Hant" data-theme="light">\n<head>\n' +
    ...
    '<script src="/assets/account.js?v=20260715a"></script>\n</body>\n</html>\n';
}
