...
const PG_CSS = `
  ...
  .pg{position:relative;display:flex;border:1px solid var(--line);border-radius:14px;background:var(--card);overflow:hidden;
      height:calc(100vh - 156px);height:calc(100dvh - 156px);min-height:420px}
  /* ---- 左側：對話列表 ---- */
  ...
  .pg-top select{border:1px solid var(--line);background:var(--field);color:var(--fg);border-radius:8px;padding:8px 10px;
                 font-size:13px;font-weight:600;font-family:inherit;outline:none;max-width:100%;min-width:0}
  ...
  .pg-hero .big{font-size:40px;margin-bottom:8px}
  .pg-hero h2{font-size:19px;margin:0 0 8px}
  .pg-hero p{font-size:13.5px;color:var(--muted);line-height:1.75;margin:0 0 16px}
  .pg-sugs{display:flex;gap:8px;flex-wrap:wrap;justify-content:center}
  .pg-sug{border:1px solid var(--line);background:var(--card);color:var(--fg);border-radius:18px;padding:8px 15px;
          font-size:12.5px;font-weight:600;cursor:pointer;font-family:inherit;transition:.15s}
  .pg-sug:hover{border-color:var(--line2)}
  /* 輸入區 */
  ...
  @media(max-width:839.98px){
    .pg{height:calc(100vh - 140px);height:calc(100dvh - 140px)}
...
export async function playgroundPageResponse(env) {
  ...
  return html(pageShell({
    ...
    headExtra: "<style>" + MEMBER_CSS + PG_CSS + "</style>\n",
    body: body
  }));
}
...
(function(){
  ...
  function buildApp(){
    ...
    UI.ta.rows=1;
    UI.ta.placeholder=groups.length?tx("輸入訊息…（Enter 送出、Shift+Enter 換行）","Message… (Enter to send, Shift+Enter for newline)"):tx("目前沒有可用的模型","No models available");
  ...
  function renderMsgs(){
    UI.msgs.innerHTML="";
    if(!msgs.length){UI.msgs.appendChild(hero());return;}
  ...
  function hero(){
    ...
    h.appendChild(el("div","big","\\u2728"));
    h.appendChild(el("h2",null,tx("想聊點什麼？","What shall we explore?")));
    if(!groups.length){
    ...
    h.appendChild(el("p",null,tx("選好上面的模型直接開聊。對話會存在你的帳號裡，換裝置也看得到。","Pick a model above and start chatting. Conversations are saved to your account.")));
    var sg=el("div","pg-sugs");
    [tx("用簡單的話解釋什麼是 API","Explain APIs in simple terms"),
     tx("幫我想 5 個週末小旅行的點子","Give me 5 weekend trip ideas"),
     tx("寫一首關於下雨天的短詩","Write a short poem about rain")].forEach(function(s){
      var b=el("button","pg-sug",s);
      b.addEventListener("click",function(){UI.ta.value=s;autoGrow();UI.ta.focus();});
      sg.appendChild(b);
    });
    h.appendChild(sg);
    return h;
