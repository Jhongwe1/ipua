...
| 16 | **斷線後 waitUntil 只有 30 秒是推論、不是實測**（ADR-0012） | 30 秒是在「正常跑完」的串流上量到的（69 秒、客戶端還連著），斷線情境沒實測；已用 `BG.ckMs` 每 5 秒存檔把猜錯的代價封頂 | 用 `wrangler tail` 實測斷線後的實際存活時間 → 回頭校準 `BG.budgetMs`（量到更長就調高，完整率直接提升） |
