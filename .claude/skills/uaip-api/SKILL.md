...
改到程式碼（而非內容）時：改了 API 要**三件套同步** — `API.md`＋`docs/openapi.yaml`，
再跑 `npm run apidoc` 與 `npm run openapi`（`src/lib/apidoc.ts`／`src/lib/openapi.ts` 都是自動產生的，
不要手改）；部署 `npm run deploy`。CI 有路由表×openapi 雙向核對，漏寫會紅燈。
