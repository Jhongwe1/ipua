...
|---|---|---|---|---|---|
| Shape | Pages Functions + 1 D1 | Go server + MySQL/Redis | Python proxy + DB | Hosted SaaS | Managed edge in front of your keys |
| Deploy | `wrangler pages deploy` | container + DB | container/pip + DB | nothing (it's hosted) | dashboard config |
...
| Billing/credits | none (quota by request count) | full credit system | usage tracking, budgets | real credits/pricing | usage analytics |
| Metering | req_log rows, p50/p95, tokens | extensive | extensive | dashboards | dashboards |
...
- **Billing in money** (OpenRouter, one-api): real credit systems with per-model pricing.
  uaip counts requests and records tokens but never prices them (DEBT #10).
...
- **Fully owned, fully readable.** Every line that runs in production is in this repo and
  ...
  or read the traffic.
- **Honest engineering evidence.** 170+ tests in the real runtime, a threat model, ADRs, and
...
infrastructure they fully own and can reason about end-to-end.* For that, a 6k-line
...
稽核一體、全碼自持可審、可論述的工程證據（170+ 測試＋威脅模型＋ADR）。
